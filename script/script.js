...
html.addEventListener('click', () => {
  ...
  if(user.password === req.body.password) {<br>req.session.username = req.body.username<br>req.session.userId = user.dataValues.id<br>req.session.authenticated = true<br>res.redirect('/')<br>}<br>})<br>})<br><br>app.post('/blog', upload.single('postimg'), (req,res) => {<br>models.post.create({<br>title: req.body.title,<br>body: req.body.body,<br>author: req.body.author,<br>image: req.file.path<br>})<br>})<br><br>app.listen(3000, () => {<br>console.log('You are live')<br>})<br><br></code>`
})

css.addEventListener('click', () => {
  code.innerHTML = `<code>CSS Text</code>`
})

script.addEventListener('click', () => {
code.innerHTML = `<code>Script Text</code>`
})
